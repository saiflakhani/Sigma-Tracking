...
def predictValues():
	...
	probability = classifier.predict_proba(valX)
	print("Probability values are : ")
	print(probability)
	counterValue = 0
	for rowVal in probability:
		thresholdFlag = False
		for probabilityValue in rowVal:
			if(probabilityValue>=0.145):
				thresholdFlag = True
		if(thresholdFlag==False):
			counterValue = counterValue+1
	print("Confidence low : "+str(counterValue)) 
	probDF = pandas.DataFrame(probability)
	print(probDF.shape)
	#print("Predictions == ")
	#print(mypred)
	lookupObject = {}
