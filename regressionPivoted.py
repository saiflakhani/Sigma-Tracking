...
# Import the dataset
dataset = pandas.read_csv('myfile.csv')


#dataset = dataset[dataset['rssiVal']<101]
#dataset['relayNo'] = dataset['relayNo'].astype('category')
x = dataset.iloc[1:, 3:25].values
y = dataset.iloc[1:, 26:].values
...
plot.plot(xTrain, linearRegressor.predict(xTrain), color = 'blue',alpha=0.01)
plot.scatter(xTrain[:,0], yTrain, color = 'red')
